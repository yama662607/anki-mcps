...
describe("MCP server", () => {
  ...
  it("executes the note-centric review workflow through MCP tools", async () => {
    ...
    try {
      ...
      const upsertNoteType = parseToolResult(
        await context.client.callTool({
          ...
          arguments: {
            ...
            dryRun: false,
            fields: [{ name: "Prompt" }, { name: "Answer" }, { name: "DetailedExplanation" }],
            templates: [
              {
                name: "Card 1",
      ...
      expect(searched.notes.map((note: { noteId: number }) => note.noteId)).toContain(
        added.note.noteId
      );
      ...
      const reviewedSearch = parseToolResult(
        await context.client.callTool({
          name: "search_notes",
          arguments: {
            profileId: "default",
      ...
      expect(reviewedSearch.notes.map((note: { noteId: number }) => note.noteId)).toContain(
        added.note.noteId
      );

      const released = parseToolResult(
        await context.client.callTool({
      ...
      const deletedAgain = parseToolResult(
        await context.client.callTool({
          name: "delete_note",
          arguments: {
            profileId: "default",
  ...
  it("supports batch add/delete with mixed outcomes", async () => {
    ...
    try {
      ...
      expect(added.results.find((item: { itemId: string }) => item.itemId === "ok-1")?.ok).toBe(
        true
      );
      expect(added.results.find((item: { itemId: string }) => item.itemId === "bad-1")?.ok).toBe(
        false
      );

      const createdNoteId = added.results.find((item: { itemId: string }) => item.itemId === "ok-1")
        ?.note.noteId;
      const deleted = parseToolResult(
        await context.client.callTool({
      ...
      expect(
        deleted.results.find((item: { itemId: string }) => item.itemId === "delete-created")?.status
      ).toBe("deleted");
      expect(
        deleted.results.find((item: { itemId: string }) => item.itemId === "delete-missing")?.status
  ...
  it("exposes note-type lint validation through MCP on dry-run", async () => {
    ...
    try {
      ...
      expect(
        result.result.validation.errors.map((issue: { code: string }) => issue.code)
      ).toContain("UNKNOWN_FIELD_REF");
      expect(
        result.result.validation.errors.map((issue: { code: string }) => issue.code)
      ).toContain("INVALID_CSS_SYNTAX");
    } finally {
      await closeContext(context);
    }
  });
