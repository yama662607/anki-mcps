...
describe("AnkiConnectGateway", () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (_url: string, init?: RequestInit) => {
        ...
        if (body.action === "notesInfo")
          return ok([{ noteId: 1001, mod: 999, fields: {}, tags: [], modelName: "Basic" }]);
  ...
  it("lists note types with lightweight summary requests instead of full schema fan-out", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (_url: string, init?: RequestInit) => {
        ...
        if (body.action === "modelFieldNames" && body.params.modelName === "Basic")
          return ok(["Front", "Back"]);
        if (body.action === "modelFieldNames" && body.params.modelName === "Cloze")
  ...
  it("creates and updates note types through model actions", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (_url: string, init?: RequestInit) => {
        ...
        if (
          body.action === "modelNames" &&
          calls.filter((item) => item.action === "modelNames").length === 1
        ) {
          return ok(["Basic"]);
        }
    ...
    await gateway.upsertNoteType({
      ...
      templates: [
        { name: "Card 1", front: "{{Code}}", back: "{{FrontSide}}<hr id=answer>{{Expected}}" },
      ],
  ...
  it("stores media files and lists media names through AnkiConnect actions", async () => {
    ...
    const stored = await gateway.storeMediaFile({
      filename: "mcp-audio-deadbeef.mp3",
      path: "/tmp/test.mp3",
    });
