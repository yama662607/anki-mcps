import type { NoteTypeField, NoteTypeTemplate, NoteTypeValidation, NoteTypeLintIssue } from '../contracts/types.js';
  ...
  for (const name of names) {
    if (seen.has(name)) {
      target.push({
        ...
        location: { side: 'note_type', fieldName: label === 'field' ? name : undefined, templateName: label === 'template' ? name : undefined },
      });
      continue;
    }
    seen.add(name);
...
function normalizeFieldName(rawToken: string): string {
  ...
  const segments = withoutPrefix.split(':').map((segment) => segment.trim()).filter((segment) => segment.length > 0);
