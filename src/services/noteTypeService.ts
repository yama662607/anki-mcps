...
export class NoteTypeService {
  ...
  async upsertNoteType(input: {
    ...
    const applied = await this.ankiGateway.upsertNoteType({
      ...
      css: targetSchema.css,
      isCloze: targetSchema.isCloze,
      newFieldNames: operations
        .filter((operation) => operation.kind === "add_field")
        .map((operation) => operation.fieldName),
      newTemplates: operations
        .filter((operation) => operation.kind === "add_template")
        .map((operation) => {
          const template = targetSchema.templates.find(
            (item) => item.name === operation.templateName
          );
  ...
  private planOperations(
    existing: NoteTypeSchema | undefined,
    target: NoteTypeSchema
  ): NoteTypeUpsertOperation[] {
    if (!existing) {
    ...
    if (existing.isCloze !== target.isCloze) {
      throw new AppError(
        "FORBIDDEN_OPERATION",
        "Changing cloze/non-cloze mode is not supported in Phase2"
      );
    }
    ...
    for (const fieldName of existingFieldNames) {
      if (!targetFieldNames.includes(fieldName)) {
        throw new AppError(
          "FORBIDDEN_OPERATION",
          `Removing or renaming field is not allowed in Phase2: ${fieldName}`
        );
      }
    }

    const existingTemplatesByName = new Map(
      existing.templates.map((template) => [template.name, template])
    );
    ...
    for (const templateName of existingTemplatesByName.keys()) {
      if (!targetTemplateNames.includes(templateName)) {
        throw new AppError(
          "FORBIDDEN_OPERATION",
          `Removing or renaming template is not allowed in Phase2: ${templateName}`
        );
      }
    }
