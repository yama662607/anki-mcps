    ...
    JSON.stringify(
      {
        ...
        nextStep:
          "Inspect the preview in Anki, then rerun with ANKI_E2E_SCENARIO=finalize and ANKI_E2E_FINALIZE=update|unsuspend|delete.",
      },
      null,
      2
    )
    ...
    JSON.stringify(
      {
        ...
        nextStep:
          "Inspect the preview in Anki, then rerun with ANKI_E2E_SCENARIO=finalize and ANKI_E2E_FINALIZE=unsuspend|delete.",
      },
      null,
      2
    )
    ...
    if (finalize === "unsuspend") {
      result = await Promise.all(
        state.notes.map((note) =>
          callTool("set_note_cards_suspended", {
